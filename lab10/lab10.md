...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Всегда изменяются Identification и Header checksum
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняются: verion, header length, differentiated services field, total length, flags, fragment offset, protocol, source address, destination address.
      - Неизменными обязательно должны оставаться total length (так как мы установили ее в 56 при запуске traceroute), protocol, source address, destination address.
      - Должны изменяться identification, header checksum (контрольная сумма меняется как минимум потому, что меняется identification) и Time to Live (однако он изменяется не с каждым следующим сообщением, а с каждым третьим, т.к. traceroute отправляется по три пакета на каждое значение TTL).
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Оно возрастает на 1 с каждым следующим отправленным пакетом.
      - Да, закономерность есть, так как это поле содержит номер отправляемого пакета, и у каждого следующего пакета он на 1 больше, чем у предыдущего.
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
   - Identification: 31331.
   - TTL: 1.  
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Значения identification разные у всех трех пакетов, пришедших с первого транзитного маршрутизатора, а TTL у них одинаковый (64), но отличается от TTL пакетов, отправленных моим хостом (56).  
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Да, сообщение было разбито на 3 IP-дейтаграммы  
      ![alt text](image-7.png)
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Изменяются Fragment Offset, Тotal Length, Flags, Header Checksum  
