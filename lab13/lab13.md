...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0xcfe47f26 (см. скриншот выше). Transaction-ID - это уникальный идентификатор транзакции, по которому клиент может распознать предназначенный именно ему ответ от сервера
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - В dhcp-запросе source address 0.0.0.0, destination address 255.255.255.255 (broadcast). В dhcp-ответе source address 192.168.0.1, destination address 192.168.0.19
5. Каков IP-адрес вашего DHCP-сервера?
   - 192.168.0.1  
   ![alt text](image-2.png)
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - Срок аренды IP-адреса - это промежуток времени, в течение которого хост может пользоваться выданным ему dhcp-сервером адресом. Если хост не продлит этот адрес до окончания срока аренды, то по истечении этого срока он потеряет этот адрес и будет вынужден запросить новый. В данном эксперимерте срок аренды составляет 1 сутки  
...
#### Демонстрация работы
Код находится в файле traffic_all.py  
...
#### Демонстрация работы
Код находится в файле traffic_ports.py (для запуска требуются права sudo).  
...
#### Демонстрация работы
Код находится в файле scan_network.py (для запуска требуются права sudo).  
