...
1. Использует ли ваш браузер HTTP версии 1.0 или 1.1? Какая версия HTTP работает на
   сервере?
   - Мой браузер использует HTTP/1.1<br>
   ![alt text](image.png)
   - На сервере работает HTTP/1.1<br>
...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Браузер может принимать английский (британский и американский) и русский языки.<br>
   ![alt text](image-2.png)
   - Также браузер предоставляет информацию о типах контента (HTTP-хедер Accept) и типах кодировки контента (HTTP-хедер Accept-Encoding), которые он может принимать<br>
   ![alt text](image-3.png)
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - IP-адрес моего компьютера 192.168.43.198
   - IP-адрес сервера 128.119.245.12<br>
   ![alt text](image-4.png)
4. Какой код состояния возвращается с сервера на ваш браузер?
   - Возвращается код 200<br>
...
1. Проверьте содержимое первого HTTP-запроса GET. Видите ли вы строку «IF-MODIFIED-SINCE» в HTTP GET?
   - Строки «IF-MODIFIED-SINCE» нет.<br>
   ![alt text](image-8.png)
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Сервер вернул содержимое файла явно.
   - Это видно по значению хедера Content-Length, равному 371 байт.<br>
...
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Во втором запросе есть строка «IF-MODIFIED-SINCE». За ней следует информация о дате и времени (Tue, 28 Oct 2025 05:59:01 GMT)<br>
...
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - 304 Not Modified
   - Сервер не вернул явно содержимое файла (отсутствует хедер Content-Length)
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - Браузер отправил один GET (не считая запрос GET на получение /favicon.ico)
   - Номер пакета 420
...
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Нет, так как HTTP и TCP работают на разных уровнях сетевой модели.<br>
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - Было отправлено 3 GET-запроса (не считая запрос GET на получение /favicon.ico)
   - Первые два были отправлены на адрес 128.119.245.12, третий - на адрес 2.56.99.24
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Изображения были загружены параллельно, так как сначала были отправлены оба запроса GET и только потом пришло оба ответа на них. Кроме того, на уровне протокола TCP пакеты передавались по двум разным потокам.
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - Включается Authorization:Credentials
